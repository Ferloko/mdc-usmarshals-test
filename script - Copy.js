document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load fichas on page load for immediate availability
    loadFichas();
    ...
    function setupToolbarButtons() {
        ...
        toolbarButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Handle different toolbar button actions
                if (title === 'Inicio') {
                    // Close all panels and show start menu
                    closeAllPanels();
                    startMenu.classList.add('active');
                } else if (title === 'Crear ficha') {
                    closeAllPanels();
                    formModal.classList.add('active');
                } else if (title === 'Buscar ficha') {
                    closeAllPanels();
                    searchModal.classList.add('active');
                } else if (title === 'Crear reporte de arresto') {
                    closeAllPanels();
                    ...
                    // Clear personal info fields when opening from toolbar (not from profile)
                ...
                } else if (title === 'Código Penal') {
                    closeAllPanels();
                    codigoPenalModal.classList.add('active');
                } else if (title === 'Crear ByC') {
                    closeAllPanels();
                ...
                } else if (title === 'Crear BOLO') {
                    closeAllPanels();
                    boloModal.classList.add('active');
                } else if (title === 'Buscar') {
                    togglePanel(searchPanel);
                    setTimeout(() => searchInput.focus(), 100);
                } else if (title === 'Zoom') {
                    showNotification('Zoom', 'Funcionalidad de zoom próximamente', 'info');
                } else if (title === 'Notificaciones') {
                    togglePanel(notificationsPanel);
                } else {
                    showNotification('Funcionalidad', `Abriendo: ${title}`, 'info');
                }
            });
        });
    }
    
    // Initialize toolbar buttons after DOM is loaded
    setupToolbarButtons();
...
window.setupToolbarButtons = function() {
    ...
    // Setup toolbar buttons - moved inside DOMContentLoaded
        ...
        button.addEventListener('click', function(e) {
            ...
            switch (title) {
                ...
                case 'Crear reporte de arresto': {
                    ...
                    // Reset and prepare arrest form
                    const _arrestForm = document.getElementById('arrestForm');
                    if (_arrestForm) {
                        _arrestForm.reset();
                    }
                    ...
                    const _now = new Date();
                    const _fechaInput = document.getElementById('fechaArresto');
                    const _horaInput = document.getElementById('horaArresto');
                    if (_fechaInput) _fechaInput.value = _now.toISOString().split('T')[0];
                    if (_horaInput) _horaInput.value = _now.toTimeString().split(' ')[0].substring(0, 5);
        
                    // Clear personal info fields
                    const _nombreInputArrest = document.getElementById('arrestNombre');
                    const _idInputArrest = document.getElementById('arrestIdentificacion');
                    if (_nombreInputArrest) _nombreInputArrest.value = '';
                    if (_idInputArrest) _idInputArrest.value = '';
        
                    // Setup autocomplete for arrest form (use global helpers if available)
                    if (window.allFichas && window.allFichas.length === 0 && typeof window.loadFichas === 'function') {
                        window.loadFichas()
                            .then(() => { if (typeof window.setupArrestAutocomplete === 'function') window.setupArrestAutocomplete(); })
                            .catch((err) => { console.error('Error loading fichas for arrest autocomplete:', err); if (typeof window.setupArrestAutocomplete === 'function') window.setupArrestAutocomplete(); });
                    } else {
                        if (typeof window.setupArrestAutocomplete === 'function') window.setupArrestAutocomplete();
                    }
                ...
                case 'Crear ByC': {
                    ...
                    // Reset and prepare ByC form
                    const _bycForm = document.getElementById('bycForm');
                    if (_bycForm) _bycForm.reset();
        
                    // Clear specific fields (use unique local names)
                    const _nombreInputByC = document.getElementById('bycNombreCompleto');
                    const _idInputByC = document.getElementById('bycIdentificacion');
                    const _descInputByC = document.getElementById('bycDescripcionHechos');
                    if (_nombreInputByC) _nombreInputByC.value = '';
                    if (_idInputByC) _idInputByC.value = '';
                    if (_descInputByC) _descInputByC.value = '';
        
                    // Setup autocomplete for ByC form
                    if (window.allFichas && window.allFichas.length === 0 && typeof window.loadFichas === 'function') {
                        window.loadFichas()
                            .then(() => { if (typeof window.setupByCAutocomplete === 'function') window.setupByCAutocomplete(); })
                            .catch((err) => { console.error('Error loading fichas for ByC autocomplete:', err); if (typeof window.setupByCAutocomplete === 'function') window.setupByCAutocomplete(); });
                    } else {
                        if (typeof window.setupByCAutocomplete === 'function') window.setupByCAutocomplete();
                    }
                ...
                case 'Crear BOLO': {
                    ...
                    // Reset BOLO form
                    const _boloForm = document.getElementById('boloForm');
                    if (_boloForm) {
                        _boloForm.reset();
                        // clear fields if present
                        const _boloNombre = document.getElementById('boloNombreCompleto');
                        const _boloId = document.getElementById('boloIdentificacion');
                        const _boloDesc = document.getElementById('boloDescripcion');
                        if (_boloNombre) _boloNombre.value = '';
                        if (_boloId) _boloId.value = '';
                        if (_boloDesc) _boloDesc.value = '';
                    }
                ...
                case 'BOLOs Activos': {
                    ...
                    if (typeof loadBolosActivosModal === 'function') loadBolosActivosModal();
                    break;
                }
                ...
                case 'C\u00f3digo Penal': {
                    ...
                    if (typeof loadCodigoPenal === 'function') loadCodigoPenal();
                    break;
                }
...
window.showCodigoPenalContent = function(articulosData, articuloNum) {
    ...
    // ===== CORRECCIÓN: mapear cargos a tarjetas =====
